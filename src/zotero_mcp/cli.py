...
def main():
    ...
    update_db_parser.add_argument("--pipelines", type=int,
                                 help="Concurrent extract→embed pipelines for --fulltext (overrides config/env)")
    ...
    update_db_parser.add_argument("--docling-gpu-ids",
                                 help="Comma-separated GPU ids/devices for Docling extraction workers (e.g. 0,1 or cuda:0,cuda:1) (overrides config/env)")
    update_db_parser.add_argument("--embedding-device",
                                 help="Device for local embeddings (e.g. cpu, cuda, cuda:0, or 0) (overrides config/env)")
    ...
    update_db_parser.add_argument("--embedding-chunk-size", type=int,
                                 help="Chunk size for local embeddings (overrides config/env)")
    update_db_parser.add_argument("--embedding-devices",
                                 help="Comma-separated devices for multi-GPU embeddings (e.g. cuda:0,cuda:1 or 0,1) (overrides config/env)")
    ...
    if args.command == "version":
        ...
        if getattr(args, "pipelines", None):
            if args.pipelines > 0:
                os.environ["ZOTERO_PIPELINES"] = str(args.pipelines)
